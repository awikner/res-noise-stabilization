...
def main(argv):
    ...
    noisevals = np.logspace(-4, 0, num = 13, base = 10)[5:9]
    ...
    for j, rho in enumerate(rhos):
        folder = '%s_more_noisetest_rho%0.1f_sigma%1.1e_leakage%0.3f_win_%s_bias_%s_tau%0.2f_%dnodes_%dtrain_%dreals_noisetype_%s_traintype_%s/' % (system, rho, best_sigma_rho[j], best_leakage_rho[j], win_type, bias_type, tau,res_size, train_time, noise_realizations, noisetype, traintype)
