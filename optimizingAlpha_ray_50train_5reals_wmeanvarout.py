...
@ray.remote
def find_stability(noise, train_time, res_size, res_per_test, noise_realizations, num_tests, alpha_values):
    rkTime_test = 400
    split_test  = 2000
...
def main(argv):
    ...
    tic = time.perf_counter()
    results = [find_stability.remote(noise, train_time, \
            res_size, res_per_test, noise_realizations, num_tests, alpha_values) for noise in noise_values_array]
    ...
    mean_sum_squared = []
    variances = []
    for i in range(noise_values_array.size):
    ...
    for i in range(noise_values_array.size):
        np.savetxt(foldername+'mean_sum_squared_%dnodes_%dtrainiters_%dnoisereals_noise%e_raytest.csv' \
                %(res_size, train_time, noise_realizations, noise_values_array[i]), mean_sum_squared[i]  , delimiter = ',')
        np.savetxt(foldername+'variances_%dnodes_%dtrainiters_%dnoisereals_noise%e_raytest.csv' \
             %(res_size, train_time, noise_realizations, noise_values_array[i]), variances[i], delimiter = ',')
