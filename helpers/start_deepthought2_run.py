...
def start_deepthought2_run(system = 'KS', traintype = 'normal', noisetype = 'gaussian', \
        ...
        import_noise = False, reg_train_fracs = 1, discard_time = 500):
    ...
    options_str = '--resonly=%s --savepred=%s --system=%s --noisetype=%s --traintype=%s -r %d --rho=%f --sigma=%f --leakage=%f --win_type=%s --bias_type=%s --tau=%f -N %d -T %d --res=%d --tests=%d --trains=%d --debug=%s --squarenodes=%s --metric=%s --returnall=%s --savetime=%s --noisevals=%s --regvals=%s --maxvt=%d --machine=%s --parallel=%s --importres=%s --importtrain=%s --importtest=%s --importnoise=%s --regtrainfracs=%d --discardlen=%d' % (resonly_str, savepred_str, system, noisetype, traintype, noise_realizations,  rho,sigma, leakage, win_type, bias_type, tau, res_size, trainlen, num_res, num_tests, num_trains,
            debug_str, squarenodes_str, metric, returnall_str,
            savetime_str, noise_values_str, reg_values_str, max_valid_time, machine, parallel_str, import_res_str, import_train_str, import_test_str, import_noise_str, reg_train_fracs, discard_time)
