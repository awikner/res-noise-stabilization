...
def getnoisecorrection(noise, dXdu):
    corr = np.zeros((3+dXdu.shape[0],3+dXdu.shape[0]))
    M    = noise.shape[1]
    for i in range(dXdu.shape[2]):
        M_mat = np.append(dXdu[:,:,i] @ noise[:,:,i],noise[:,:,i], axis = 0)
        corr += M_mat @ M_mat.T
    corr = corr/M
    return corr
...
def get_states_wrapped(u_arr_train, u_arr_train_noise, res_X, res_dXdu, Win, W, noise_scaling, skip, noise_realizations):
    ...
    np.random.seed(10)
    noise = np.random.randn(3,noise_realizations,u_arr_train.shape[1])*noise_scaling
    ...
    states_trstates[1:,1:] = states_trstates[1:,1:] + lin_noise_correction
